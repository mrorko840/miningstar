...
{
    $frontEnds = Frontend::activeTemplate();
    if ($singleQuery) {
        $content = $frontEnds->where('data_keys', $dataKeys)->orderBy('id', 'desc')->first();
    } else {
        $frontEnds = $frontEnds->when($limit != null, function ($q) use ($limit) {
            return $q->limit($limit);
        });
        if ($orderById) {
            $content = $frontEnds->where('data_keys', $dataKeys)->orderBy('id')->get();
        } else {
            $content = $frontEnds->where('data_keys', $dataKeys)->orderBy('id', 'desc')->get();
        }
    }
    return $content;
}
...
function levelCommission($user, $amount, $trx)
{
    ...
    while ($i <= $level) {
        ...
        $transactions[] = [
            ...
            'charge'       => 0,
            'trx_type'     => '+',
            'details'      => 'You have received referral commission',
            'trx'          => $trx,
