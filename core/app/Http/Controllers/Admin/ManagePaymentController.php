...
class ManagePaymentController extends Controller
{
    ...
    {
        if ($scope) {
            $deposits = Deposit::where('order_id', '!=', Status::NOT_ORDER)->$scope()->with(['user', 'gateway']);
        } else {
            $deposits = Deposit::where('order_id', '!=', Status::NOT_ORDER)->with(['user', 'gateway']);
        }
    ...
    public function details($id)
    {
        ...
        $pageTitle = $deposit->user->username . ' paid ' . showAmount($deposit->amount) . ' ' . $general->cur_text;
