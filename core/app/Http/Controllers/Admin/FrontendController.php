...
class FrontendController extends Controller
{
    ...
    public function frontendSections($key)
    {
        ...
        $content   = Frontend::activeTemplate()->where('data_keys', $key . '.content')->orderBy('id', 'desc')->first();
        $elements  = Frontend::activeTemplate()->where('data_keys', $key . '.element')->orderBy('id')->orderBy('id', 'desc')->get();
    ...
    public function frontendContent(Request $request, $key)
    {
        ...
        if ($request->id) {
            ...
            if ($key != 'seo') $content = $content->activeTemplate();
            $content = $content->where('data_keys', $key . '.' . $request->type)->first();
