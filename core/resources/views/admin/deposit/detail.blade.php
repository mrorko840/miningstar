    ...
    <div class="row mb-none-30 justify-content-center">
        <div class="col-xl-4 col-md-6 mb-30">
            <div class="card b-radius--10 box--shadow1 overflow-hidden">
                <div class="card-body">
                    <h5 class="text-muted mb-20">@if($deposit->order_id) @lang('Payment Via') @else @lang('Deposit Via') @endif {{ __(@$deposit->gateway->name) }}</h5>
                    <ul class="list-group">
                ...
                <div class="card b-radius--10 box--shadow1 overflow-hidden">
                    <div class="card-body">
                        ...
                        @if ($deposit->status == Status::PAYMENT_PENDING)
                            <div class="row mt-4">
                                <div class="col-md-12">
                                    <button class="btn btn-outline--success btn-sm ms-1 confirmationBtn" data-action="@if ($deposit->order_id) {{ route('admin.payment.approve', $deposit->id) }} @else {{ route('admin.deposit.approve', $deposit->id) }} @endif" data-question="@lang('Are you sure to approve this transaction?')"><i class="las la-check-double"></i>
                                        @lang('Approve')
                                    </button>
    ...
    <div class="modal fade" id="rejectModal" role="dialog" tabindex="-1">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="@if ($deposit->order_id) {{ route('admin.payment.reject') }} @else {{ route('admin.deposit.reject') }} @endif" method="POST">
                    @csrf
