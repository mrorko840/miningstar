                            ...
                            <tbody class="list">
                                @forelse($orders as $order)
                                    <tr>
                                        ...
                                        <td> {{ showAmount($order->amount) }} {{ $general->cur_text }} </td>
                                        <td> {{ $order->plan_details->period }}</td>
                                        <td>
                                            {{ showAmount($order->min_return_per_day, 8) }} - {{ showAmount($order->max_return_per_day, 8) }} <strong>{{ $order->miner->coin_code }}</strong>
                                        </td>
                                        <td>
                                            @php
                                        ...
                                        <td>
                                            <button class="btn btn-outline--primary btn-sm detailBtn" data-status_badge="{{ $order->statusBadge }}" data-order="{{ $order }}" data-purchase_date="{{ showDateTime($order->created_at) }}" data-plan_title="{{ __($order->plan_details->title) }}" data-plan_price="{{ showAmount($order->amount) }} {{ __($general->cur_text) }}" data-status="{{ $order->status }}" data-deposit_id="{{ @$order->deposit->id }}" @if ($order->status != Status::ORDER_UNPAID) data-payment_via="{{ @$order->deposit ? $order->deposit->gateway->name : 'Wallet Balance' }}" @endif><i class="las la-desktop"></i>@lang('Detail')</button>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
