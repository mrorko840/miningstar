            ...
            <div class="col-lg-12 mrb-30">
                <div class="cal-area text-center">
                    <div class="cal-area-inner">
                        <div class="cal-title mb-2">
                            <h3 class="title text-color-theme">{{ __(@$content->data_values->heading) }}</h3>
                        </div>
                        <form class="cal-form mx-3">
                            <div class="cal-wrapper d-flex align-items-center justify-content-center flex-wrap">
                                <div class="cal-select form-group px-1">
                                    <select class="nic-select form-select form-select-sm rounded-pill" name="miner">
                                        <option value="" selected disabled>@lang('Select Coin')</option>
                                        @foreach ($miners as $miner)
                                            <option data-coin_code="{{ $miner->coin_code }}" data-plans="{{ $miner->plans }}" value="{{ $miner->id }}">{{ __($miner->name) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="cal-select plans form-group px-1">
                                    <select class="revenue-calculate nic-select form-select form-select-sm rounded-pill">
                                        <option value="" selected disabled>@lang('Select Plan')</option>
                                    </select>
                                </div>
                                <div class="revenue-area mt-2">
                                    <span class="sub-title"></span>
                                    <h3 class="title text-color-theme">
    ...
    <script>
        ...
        (function($) {
            ...
            $('select[name="miner"]').on('change', function() {
                ...
                var output = `<select class="revenue-calculate form-select form-select-sm rounded-pill"> <option value="" selected disabled>@lang('Select Plan')</option>`;
